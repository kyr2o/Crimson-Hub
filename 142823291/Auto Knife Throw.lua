    ...
    Environment.CRIMSON_AUTO_KNIFE = Environment.CRIMSON_AUTO_KNIFE or { enabled = false }
    ...
    function Environment.CRIMSON_SILENT_KNIFE.enable() Environment.CRIMSON_SILENT_KNIFE.enabled = true end
    function Environment.CRIMSON_SILENT_KNIFE.disable() Environment.CRIMSON_SILENT_KNIFE.enabled = false end
    ...
    pcall(function()
        if Environment.CRIMSON_UI and Environment.CRIMSON_UI.addButton then
            Environment.CRIMSON_UI.addButton("Silent Knife", function(state)
                if state then Environment.CRIMSON_SILENT_KNIFE.enable() else Environment.CRIMSON_SILENT_KNIFE.disable() end
            end, Environment.CRIMSON_SILENT_KNIFE.enabled, { category = "Actions" })
        end
    end)
    ...
    local function clampValue(v,a,b) if b<a then a,b=b,a end if v~=v then return a end return math.clamp(v,a,b) end
    local function isFinite(v) return v and v.X==v.X and v.Y==v.Y and v.Z==v.Z end
    ...
    local function worldVel(ch) local p = getAimPart(ch) return p and p.AssemblyLinearVelocity or Vector3.zero end
    ...
    local function performSilentThrow(origin, aimPoint, targetCharacter)
        ...
        task.delay(0.06, function()
            ...
            if not okFinish then return end
    ...
    local function step()
        ...
        local targetHum = tc and tc:FindFirstChildOfClass("Humanoid")
        local anchor = tc and getAimPart(tc)
        if not targetHum or targetHum.Health<=0 or not anchor then return end
        ...
        local pred = (function(origin,ch,focus,same,gp)
            local p=focus or getAimPart(ch)
            if not p then return nil end
            local h=ch and ch:FindFirstChildOfClass("Humanoid")
            local id,chtime=ch:GetDebugId(),os.clock()
            ...
            local predicted = Vector3.new(base.X, p.Position.Y, base.Z) + lead
            return predicted
        end)(origin, tc, targetLimb, same, gp)
        ...
        if Environment.CRIMSON_SILENT_KNIFE.enabled then
            ...
            if delayAmt > 0 then
                task.delay(delayAmt, function() performSilentThrow(origin, aim, tc) end)
            else
    ...
    function Environment.CRIMSON_AUTO_KNIFE.enable() Environment.CRIMSON_AUTO_KNIFE.enabled = true end
    function Environment.CRIMSON_AUTO_KNIFE.disable() Environment.CRIMSON_AUTO_KNIFE.enabled = false end

    pcall(function()
        if Environment.CRIMSON_UI and Environment.CRIMSON_UI.addButton and not Environment.CRIMSON_UI.has("Auto Knife Throw") then
            Environment.CRIMSON_UI.addButton("Auto Knife Throw", function(state)
                if state then Environment.CRIMSON_AUTO_KNIFE.enable() else Environment.CRIMSON_AUTO_KNIFE.disable() end
            end, Environment.CRIMSON_AUTO_KNIFE.enabled, { category = "Actions" })
        end
    end)
end

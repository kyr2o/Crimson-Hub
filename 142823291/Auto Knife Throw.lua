    Crimson Hub - Auto Knife Throw & Silent Knife
    Author: kyr2o (Original), Edited with AI
]]
...
local GITHUB_USER = "kyr2o"
local GITHUB_REPO = "Crimson-Hub"
local GITHUB_FILE_PATH = "main/142823291/autoknife_enabled.txt" 
...
local function checkGitHubFeatureFlag()
    local url = string.format("https://raw.githubusercontent.com/%s/%s/%s", GITHUB_USER, GITHUB_REPO, GITHUB_FILE_PATH)
    local ok, result = pcall(function()
        return HttpService:GetAsync(url)
    end)
    if ok and result then

        if string.lower(result):match("true") then
...
local function executeSilentKnife(targetPlayer, targetLimb)
    if not myKnife or not knifeRemote or not myCharacter then return end

    local backpack = localPlayer.Backpack
    myKnife.Parent = backpack

    task.wait(SILENT_KNIFE_DELAY) 

    myKnife.Parent = myCharacter

    local targetChar = targetPlayer.Character
    if not targetChar then return end
    local targetRoot = getAimPart(targetChar)
    if not targetRoot then return end

    local origin = myRoot.Position
    local targetPos = targetRoot.Position
    local ignoreList = { myCharacter, targetChar }

    local hit = raycastVec(origin, (targetPos - origin).Unit, (targetPos - origin).Magnitude, ignoreList)
...
local function step()
    ...
    if not Environment.CRIMSON_AUTO_KNIFE.enabled and not Environment.CRIMSON_SILENT_KNIFE.enabled then return end
    ...
    if Environment.CRIMSON_AUTO_KNIFE.enabled and not isFeatureEnabled then
    ...
    if Environment.CRIMSON_SILENT_KNIFE.enabled then
        executeSilentKnife(targetPlayer, targetLimb)
    elseif Environment.CRIMSON_AUTO_KNIFE.enabled and isFeatureEnabled then

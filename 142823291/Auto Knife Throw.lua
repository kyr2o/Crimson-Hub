...
-- [MODIFIED] Added "Silent Knife" to the Actions modules list
local CATEGORY_SPEC = {
    [MM2_PLACEID] = {
                ...
                if cat.modules ~= "REMAINDER" then
                    ...
                    for _, modName in ipairs(cat.modules) do
                        ...
                        if fn then
							...
							-- [ADDED] This is the new block for the "Silent Knife" button
							elseif modName == "Silent Knife" then
								createScriptButton(content, "Silent Knife", function(state)
									local G = (getgenv and getgenv()) or _G
									G.CRIMSON_SILENT_KNIFE = G.CRIMSON_SILENT_KNIFE or { enabled = false }
									if state then
										G.CRIMSON_SILENT_KNIFE.enabled = true
										if G.CRIMSON_SILENT_KNIFE.enable then
											pcall(G.CRIMSON_SILENT_KNIFE.enable)
										end
									else
										G.CRIMSON_SILENT_KNIFE.enabled = false
